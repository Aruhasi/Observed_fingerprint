...
client, scluster = scluster.init_dask_slurm_cluster(walltime="01:30:00")
# In[3]:
...
def main(runs):
    # Directory setup
    dir_residuals = '/work/mh0033/m301036/Land_surf_temp/Disentangling_OBS_SAT_trend/Figure2/ACCESS/'
    dir_output = '/work/mh0033/m301036/Land_surf_temp/Disentangling_OBS_SAT_trend/LE_evaluation/Fig3_ACCESS/output/'
    
    # Load dataset
    ds_ACCESS_1850_2022 = xr.open_mfdataset(
        dir_residuals + 'GSAT_ACCESS_Internal_Variability_anomalies_1850_2022.nc',
        chunks={'run': 1, 'lat': 45, 'lon': 90}
    ).isel(run=runs)
    # run = runs
    # Define segment lengths and compute trends
    segment_lengths = range(10, 74, 1)
    combined_results = compute_trend(ds_ACCESS_1850_2022, segment_lengths, func_mk)
    # Initialize a list to hold results
    # delayed_trend_results = []

    # for run in runs:  # Iterate over runs
    #     # Select the current run
    #     ds_run = ds_ACCESS_1850_2022.sel(run=run)
    ...
    output_path = os.path.join(dir_output, f'ACCESS_ICV_segments_1850_2022_run{runs}_trend.nc')
