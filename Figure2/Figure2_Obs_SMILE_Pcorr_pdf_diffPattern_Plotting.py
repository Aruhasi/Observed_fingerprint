...
# # perform T-test for Independent Models:
# # Calculate variances
# # Assume trend data and uncertainties
# obs_trend_var = HadCRUT5_trend.trend.var(dim=["lat", "lon"], skipna=True)
# model_trend_var = MMEM_annual_trend.trend.var(dim=["lat", "lon"], skipna=True)

# # Convert trends to NumPy arrays for element-wise operations
# hadcrut5_mean = HadCRUT5_trend.trend.values
# mmem_mean = MMEM_annual_trend.trend.values
# # %%
# # Compute Z-scores
# z_score = (hadcrut5_mean - mmem_mean) / np.sqrt(obs_trend_var + model_trend_var)

# # Compute p-values from Z-scores
# p_value = 2 * (1 - norm.cdf(np.abs(z_score)))  # Two-tailed test

# # Mask for significance at 95% confidence level
...
#     z_score,
#     dims=("lat", "lon"),
#     coords={"lat": HadCRUT5_trend.lat, "lon": HadCRUT5_trend.lon}
# )
...
# z_score_da.plot(cmap="coolwarm", vmin=-3, vmax=3, label="Z-score")
# plt.title("Z-Score for Trend Differences (HadCRUT5 vs MMEM)")
# plt.show()

# # Overlay significance
# plt.figure(figsize=(10, 6))
# significance_mask_da.plot(cmap="Greys", label="Significance Mask")
# plt.title("Significance Mask (95% Confidence)")
# plt.show()
# # %%
# # Mask for significance (95% confidence level)
# significance_mask = (p_value < 0.05)
# # %%
# # Reshape `significance_mask` to match the 2D grid of lat/lon
# if len(significance_mask.shape) == 1:
#     significance_mask = significance_mask.reshape(HadCRUT5_trend.trend.shape)

# # Plot the trend difference with significance overlay
# fig, ax = plt.subplots(figsize=(10, 6))

# # Plot the trend difference
...
#     levels=20
# )
# plt.colorbar(diff_plot, ax=ax, label="Trend Difference (°C/decade)")

# # Overlay the significance mask
# ax.contourf(
#     HadCRUT5_trend.lon, 
#     HadCRUT5_trend.lat, 
#     significance_mask, 
#     colors="none", 
#     hatches=["..."],  # Add hatching for significance
#     linewidths=0.5
# )

# # Add labels and title
# ax.set_title("Difference in Trends (Obs - Model) with 95% Significance")
...
fig.savefig('Fig2.jpeg', dpi=300, bbox_inches='tight')
