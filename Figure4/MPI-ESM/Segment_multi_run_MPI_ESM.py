...
def main(runs):
    ...
    ds_MPI_ESM_1850_2022 = xr.open_mfdataset(
        dir_residuals + 'GSAT_MPI_ESM_Internal_Variability_anomalies_1850_2022.nc',
        chunks={'run': 1, 'lat': 45, 'lon': 90}
    ).isel(run=runs)
    # run = runs
    # Define segment lengths and compute trends
    segment_lengths = range(10, 74, 1)
    combined_results = compute_trend(ds_MPI_ESM_1850_2022, segment_lengths, func_mk)
    # Initialize a list to hold results
    # delayed_trend_results = []

    # for run in runs:  # Iterate over runs
    #     # Select the current run
    #     ds_run = ds_MPI_ESM_1850_2022.sel(run=run)
    ...
    output_path = os.path.join(dir_output, f'MPI_ESM_ICV_segments_1850_2022_run{runs}_trend.nc')
