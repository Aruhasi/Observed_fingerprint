...
# client, scluster = scluster.init_dask_slurm_cluster(walltime="08:00:00")
# In[3]:
...
def main(runs):
    # Directory setup
    dir_residuals = '/work/mh0033/m301036/OBS_LPS_revision/docs/data/FIG3/CESM2/'
    # 100 realizations of the CESM2 internal variability
    dir_output = '/work/mh0033/m301036/OBS_LPS_revision/docs/data/FIG3/CESM2/output/'
    os.makedirs(dir_output, exist_ok=True)
    # Load dataset
    ds_CESM2_1850_2022 = xr.open_mfdataset(
            dir_residuals + 'SAT_anomaly_partition_wrt_CESM2_CMIP6_SMBB_ENS_GSAT_1850-2022.nc',
            chunks={'run': 1, 'lat': 45, 'lon': 90}
        )["internal_variability"].isel(run=runs)
    # Drop string coords that would otherwise be written as dtype=object
    drop_coords = [c for c in ["run", "realization"] if c in ds_CESM2_1850_2022.coords]
    if drop_coords:
        ds_CESM2_1850_2022 = ds_CESM2_1850_2022.reset_coords(names=drop_coords, drop=True)
    print(ds_CESM2_1850_2022)
    # exit()
    # run = runs
    # Define segment lengths and compute trends
    segment_lengths = range(10, 74, 1)
    combined_results = compute_trend(ds_CESM2_1850_2022, segment_lengths, func_mk)
    # Initialize a list to hold results
    # delayed_trend_results = []

    # for run in runs:  # Iterate over runs
    #     # Select the current run
    #     ds_run = ds_CESM2_1850_2022.sel(run=run)
    ...
    output_path = os.path.join(dir_output, f'CESM2_ICV_segments_1850_2022_run{runs}_trend.nc')
    # Ensure no leftover string coords before writing
    for coord in ("member", "realization"):
        if coord in combined_results.coords:
            combined_results = combined_results.reset_coords(names=coord, drop=True)
