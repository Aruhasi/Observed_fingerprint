...
#SBATCH --job-name=CESM2_ICV    # Job name
...
#SBATCH --time=04:30:00        # Maximum runtime (2 hours)
#SBATCH --ntasks=1            # Number of tasks
#SBATCH --cpus-per-task=64      # Number of CPU cores per task
#SBATCH --mem=256G              # Memory per task
#SBATCH --array=1-100%2           # Job array range (50 tasks)

source $(conda info --base)/etc/profile.d/conda.sh
conda activate mykernel # change the name for the environment
python Segment_multi_run_CESM2.py $SLURM_ARRAY_TASK_ID # change the name of the script