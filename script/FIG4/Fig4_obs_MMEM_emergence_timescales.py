...
import numpy as np
# %%
data_obs = xr.open_dataset("/work/mh0033/m301036/OBS_LPS_revision/docs/data/FIG4/median_SNgt1/OBS_Emergence_time_scale.nc")
...
# %%
dir_MMLE = "/work/mh0033/m301036/OBS_LPS_revision/docs/data/FIG4/median_SNgt1/"
MMEM = xr.open_dataset(dir_MMLE + "MMEM_emergence_timescale_median.nc")
# %%
# check the NaN values
print(data_obs.emergence_timescale.isnull().sum())
print(MMEM.emergence_timescale_median.isnull().sum())
# %%
...
# Compute difference on the matching variables so we can plot a DataArray
diff = MMEM['emergence_timescale_median'] - data_obs['emergence_timescale']
...
# Plot data
p_obs = data_obs['emergence_timescale'].plot(
    ax=ax,
...
ax.contourf(
    lon_with_cyclic, data_obs['emergence_timescale'].lat, ~monotonic_with_cyclic,
...
levels_diff = np.arange(-25, 30, 5)  # Symmetric around 0
n_bins = len(levels_diff)-2         # = 10 bins

p_diff = diff.plot(ax=ax1, transform=ccrs.PlateCarree(), cmap=cmap_diff, levels=levels_diff, add_colorbar=False)
# Add title
...
ax1.plot([180%360, 260%360, 260%360, 180%360, 180%360], [-55, -55, -70, -70, -55],
        color='grey', linewidth=2.5, transform=ccrs.PlateCarree())
# Extratropical South Pacific box # Jan 19 2026 version
...
for ext in ("png", "pdf"):
    fig.savefig(figure_output+f"FIG4_SNgt1_median_emergence_timescales.{ext}", dpi=300, bbox_inches='tight')
