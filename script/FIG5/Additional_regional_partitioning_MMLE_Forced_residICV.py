...
da_NorthPacific  = xr.Dataset(
    {'raw':data_array['NPI'].raw, 
    'forced': data_array['NPI'].forced, 
    'unforced': data_array['NPI'].internal})
# %%
da_SouthernOcean    = xr.Dataset({'raw':data_array['SO'].raw, 'forced': data_array['SO'].forced, 'unforced': data_array['SO'].internal})
...
print(da_NorthPacific.min(), da_NorthPacific.max())
# %%
print(da_SouthernOcean.min(), da_SouthernOcean.max())
...
NPM_unforced_lower = xr.open_dataset(f'{dir_input_percentile}internal_NPI_trend_lower_percentile.nc')
SO_unforced_lower = xr.open_dataset(f'{dir_input_percentile}internal_SO_trend_lower_percentile.nc')

NPM_unforced_upper = xr.open_dataset(f'{dir_input_percentile}internal_NPI_trend_upper_percentile.nc')
SO_unforced_upper = xr.open_dataset(f'{dir_input_percentile}internal_SO_trend_upper_percentile.nc')
# %%
...
MMLE_forced_NPI_trend = xr.open_dataset(f'{dir_MMLE_forced}MMLE_ENSforced_NPI_trend_1950-2022_sliding.nc').trend
MMLE_forced_SO_trend = xr.open_dataset(f'{dir_MMLE_forced}MMLE_ENSforced_SO_trend_1950-2022_sliding.nc').trend
# %%
# overlaid the PI-control constructed unforced trend range
PI_unforced_trend = xr.open_dataset('/work/mh0033/m301036/OBS_LPS_revision/docs/data/Revision_check/PI_control_ICV/Data/PI_Control_percentiles.nc')
PI_unforced_trend
SMILE_unforced_trend = xr.open_dataset('/work/mh0033/m301036/OBS_LPS_revision/docs/data/FIG5/SMILE_residual_ICV/MMLE_equal_model_band_1950_2022_sliding.nc')
SMILE_unforced_trend
# %%
# rename the variable name
NPM_PI_unforced_lower = PI_unforced_trend["p5"].sel(region='NPM')
SO_PI_unforced_lower = PI_unforced_trend["p5"].sel(region='SO')
NPM_PI_unforced_upper = PI_unforced_trend["p95"].sel(region='NPM')
SO_PI_unforced_upper = PI_unforced_trend["p95"].sel(region='SO')
NPM_SMILE_unforced_lower = SMILE_unforced_trend["q_low"].sel(region='NPI')
SO_SMILE_unforced_lower = SMILE_unforced_trend["q_low"].sel(region='SO')
NPM_SMILE_unforced_upper = SMILE_unforced_trend["q_high"].sel(region='NPI')
SO_SMILE_unforced_upper = SMILE_unforced_trend["q_high"].sel(region='SO')
...
# Assuming the data arrays da_NorthPacific, da_SouthernOcean, da_SoutheastPacific, da_SOP, and the bounds are defined
# Create the plot
...
linestyles = ['-', '-', '-.', ':']
titles = ['North Pacific midlatitude (NPM)', 'Southern Ocean (SO)']
vars = ['raw', 'forced', 'unforced', 'MMLE_forced']

for i, var in enumerate(vars):
    if var == 'MMLE_forced':
        da_NorthPacific[var] = MMLE_forced_NPI_trend
        da_SouthernOcean[var] = MMLE_forced_SO_trend
        sns.lineplot(x=np.arange(1950, 2014), y=da_NorthPacific[var].values, color="#B11927", linestyle=":", linewidth=5.5, ax=ax1)
        sns.lineplot(x=np.arange(1950, 2014), y=da_SouthernOcean[var].values, color="#B11927", linestyle=":", linewidth=5.5, ax=ax2)
        sns.lineplot(x=np.arange(1950, 2014), y=da_NorthPacific[var].values, color="#B11927", linestyle=":", linewidth=5.5, ax=ax3)
        sns.lineplot(x=np.arange(1950, 2014), y=da_SouthernOcean[var].values, color="#B11927", linestyle=":", linewidth=5.5, ax=ax4)
    else:
        print(f"{var} shape: ", da_NorthPacific[var].shape)
        sns.lineplot(x=np.arange(1950, 2014), y=da_NorthPacific[var].values, color=colors[i], linestyle=linestyles[i], linewidth=3.5, ax=ax1)
        sns.lineplot(x=np.arange(1950, 2014), y=da_SouthernOcean[var].values, color=colors[i], linestyle=linestyles[i], linewidth=3.5, ax=ax2)
        sns.lineplot(x=np.arange(1950, 2014), y=da_NorthPacific[var].values, color=colors[i], linestyle=linestyles[i], linewidth=3.5, ax=ax3)
        sns.lineplot(x=np.arange(1950, 2014), y=da_SouthernOcean[var].values, color=colors[i], linestyle=linestyles[i], linewidth=3.5, ax=ax4)
# Add the unforced shading region
# add the unforced shading region with reversed upper and lower bounds
ax1.fill_between(np.arange(1950,2014), NPM_unforced_lower.ICV_trend_lower.values[::-1], NPM_unforced_upper.ICV_trend_upper.values[::-1], color=colors[3])
ax2.fill_between(np.arange(1950,2014), SO_unforced_lower.ICV_trend_lower.values[::-1], SO_unforced_upper.ICV_trend_upper.values[::-1], color=colors[3])
ax3.fill_between(np.arange(1950,2014), NPM_unforced_lower.ICV_trend_lower.values[::-1], NPM_unforced_upper.ICV_trend_upper.values[::-1], color=colors[3])
ax4.fill_between(np.arange(1950,2014), SO_unforced_lower.ICV_trend_lower.values[::-1], SO_unforced_upper.ICV_trend_upper.values[::-1], color=colors[3])

# Add the unforced shading region filled with hatched lines
years = np.arange(1950, 2014)

for ax, low, high in [
    (ax1, NPM_SMILE_unforced_lower.values, NPM_SMILE_unforced_upper.values),
    (ax2, SO_SMILE_unforced_lower.values,  SO_SMILE_unforced_upper.values),
    (ax3, NPM_PI_unforced_lower.values[::-1], NPM_PI_unforced_upper.values[::-1]),
    (ax4, SO_PI_unforced_lower.values[::-1],  SO_PI_unforced_upper.values[::-1]),
]:
...
ax1.set_title(titles[0], loc='center',fontsize=32,pad=20)
ax2.set_title(titles[1], loc='center',fontsize=32,pad=20)
# ax3.set_title(titles[2], loc='left',fontsize=32,pad=20)
# ax4.set_title(titles[3], loc='left',fontsize=32,pad=20)
# Add year range text
ax1.text(1979.1, -0.96, '1979-2022', fontsize=20, rotation=90, color='#999A9E')
ax2.text(1979.1, -0.46, '1979-2022', fontsize=20, rotation=90, color='#999A9E')
ax3.text(1979.1, -0.96, '1979-2022', fontsize=20, rotation=90, color='#999A9E')
ax4.text(1979.1, -0.46, '1979-2022', fontsize=20, rotation=90, color='#999A9E')

# Add subplot order text
ax1.text(1945, 1.58, 'A', fontsize=35, ha='center', va='center', fontweight='bold')
ax2.text(1945, 0.8, 'B', fontsize=35, ha='center', va='center', fontweight='bold')
ax3.text(1945, 1.58, 'C', fontsize=35, ha='center', va='center', fontweight='bold')
ax4.text(1945, 0.8, 'D', fontsize=35, ha='center', va='center', fontweight='bold')
...
ax1.add_artist(leg1)
leg2 = ax3.legend(custom_lines, ['total', 'external forcing', 'internal variability', 'MMLE forced'], 
                  loc='lower left', fontsize=18)
ax3.add_artist(leg2)
...
plt.savefig(f'{dir_fig_output}FIG5_additional_region_PI_CNTL_MMLE_median_residICV.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{dir_fig_output}FIG5_additional_region_PI_CNTL_MMLE_median_residICV.pdf', dpi=300, bbox_inches='tight')
...
# titles = ['NPM(ARC)', 'Warming Hole(WH)', 'Southeast Pacific(SEP)', 'Southern Ocean Pacific sector(SOP)']
# linestyles = ['-', '-', '-.', ':']
...
# for i, var in enumerate(vars):
#     print(f"{var} shape: ", da_NorthPacific[var].shape)
#     da_NorthPacific[var] = da_NorthPacific[var].where(np.isfinite(da_NorthPacific[var]), np.nan)
#     da_SouthernOcean[var] = da_SouthernOcean[var].where(np.isfinite(da_SouthernOcean[var]), np.nan)
...
# ax1.fill_between(np.arange(1950,2014), NPM_unforced_lower.ICV_trend_lower.values[::-1], NPM_unforced_upper.ICV_trend_upper.values[::-1], color=colors[3])
# ax2.fill_between(np.arange(1950,2014), SO_unforced_lower.ICV_trend_lower.values[::-1], SO_unforced_upper.ICV_trend_upper.values[::-1], color=colors[3])
# ax3.fill_between(np.arange(1950,2014), SEP_unforced_lower.ICV_trend_lower.values[::-1], SEP_unforced_upper.ICV_trend_upper.values[::-1], color=colors[3])
# ax4.fill_between(np.arange(1950,2014), SOP_unforced_lower.ICV_trend_lower.values[::-1], SOP_unforced_upper.ICV_trend_upper.values[::-1], color=colors[3])
