...
from src.Statistic_cal import mmle_band_deterministic_median_of_models, mmle_band_outer_envelope_from_xarray, mmle_equal_model_band_from_xarray, mmle_equal_model_band_from_xarray
...
for model in MODELS:
    ...
    DIR_IN = f"/work/mh0033/m301036/OBS_LPS_revision/docs/data/FIG5/SMILE_residual_ICV_2013/{model}/regional_anomalies"
    files = sorted(glob.glob(f"{DIR_IN}/{model}_run*_regional_mean_trends_1950_2013_sliding.nc"))
...
# %%
# MMLE_icv_band = mmle_equal_model_band_from_xarray(
#     combined_by_model,
#     region_list=key_regions,
#     qlo=0.05, qhi=0.95,
#     K=None,          # equal-model: use min available across models for each (region,period)
#     nboot=0,
#     # seed=None,
#     varname="trend_region",
# )
# MMLE_icv_band = mmle_band_deterministic_median_of_models(
#     combined_by_model,
#     region_list=key_regions,
#     qlo=0.05, qhi=0.95,
#     varname="trend_region",
# )
# MMLE_icv_env = mmle_band_outer_envelope_from_xarray(
#     combined_by_model,
#     region_list=key_regions,
#     qlo=0.05, qhi=0.95,
#     varname="trend_region",
# )
MMLE_icv_env = mmle_equal_model_band_from_xarray(
    combined_by_model,
    region_list=key_regions,
    qlo=0.05, qhi=0.95,
    K=None,          # equal-model: use min available across models for each (region,period)
...
# %%
out_nc = "/work/mh0033/m301036/OBS_LPS_revision/docs/data/FIG5/SMILE_residual_ICV_2013/MMLE_equal_weighted_5th_95th_quantile_1950_2013_sliding.nc"
MMLE_icv_env.to_netcdf(out_nc)
print("Saved:", out_nc)
# %%
variable_name = np.arange(1950,2014)
keys = [f'{i}-2013' for i in variable_name]
...
# %%
arctic_unforced_lower = MMLE_icv_env["q_low"].sel(region='Arctic')
NAWH_unforced_lower   = MMLE_icv_env["q_low"].sel(region='Subpolar_gyre')
SEP_unforced_lower    = MMLE_icv_env["q_low"].sel(region='SoutheastPacific')
SOP_unforced_lower    = MMLE_icv_env["q_low"].sel(region='SOP')

arctic_unforced_upper = MMLE_icv_env["q_high"].sel(region='Arctic')
NAWH_unforced_upper   = MMLE_icv_env["q_high"].sel(region='Subpolar_gyre')
SEP_unforced_upper    = MMLE_icv_env["q_high"].sel(region='SoutheastPacific')
SOP_unforced_upper    = MMLE_icv_env["q_high"].sel(region='SOP')
# %%
...
plt.savefig(f'{dir_fig_output}FIG5_SMILE_icv_equal_weight_percentile.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{dir_fig_output}FIG5_SMILE_icv_equal_weight_percentile.pdf', dpi=300, bbox_inches='tight')
