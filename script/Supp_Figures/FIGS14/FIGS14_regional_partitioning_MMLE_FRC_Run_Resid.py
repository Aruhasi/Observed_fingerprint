...
# load data
dir_input = '/work/mh0033/m301036/OBS_LPS_revision/docs/data/FIG5/end_year_2013/data/'
...
for region in region_name:
    if region == 'Subpolar_gyre':
        data_array[region] = {}
        data_array[region] = xr.open_dataset(f'{dir_input}subpolar_gyre_trend_variations.nc')
    else:
        data_array[region] = {}
...
arctic_unforced_lower = xr.open_dataset(f'{dir_input_percentile}internal_Arctic_trend_lower_percentile.nc')
NAWH_unforced_lower = xr.open_dataset(f'{dir_input_percentile}internal_subpolar_gyre_trend_lower_percentile.nc')
SEP_unforced_lower = xr.open_dataset(f'{dir_input_percentile}internal_SEP_trend_lower_percentile.nc')
SOP_unforced_lower = xr.open_dataset(f'{dir_input_percentile}internal_SOP_trend_lower_percentile.nc')

arctic_unforced_upper = xr.open_dataset(f'{dir_input_percentile}internal_Arctic_trend_upper_percentile.nc')
NAWH_unforced_upper = xr.open_dataset(f'{dir_input_percentile}internal_subpolar_gyre_trend_upper_percentile.nc')
SEP_unforced_upper = xr.open_dataset(f'{dir_input_percentile}internal_SEP_trend_upper_percentile.nc')
SOP_unforced_upper = xr.open_dataset(f'{dir_input_percentile}internal_SOP_trend_upper_percentile.nc')
# %%
# input the MMLE forced trend data
dir_MMLE_forced = '/work/mh0033/m301036/OBS_LPS_revision/docs/data/FIG5/end_year_2013/MMLE/'
# load the MMLE forced trend data
MMLE_forced_ARC_trend = xr.open_dataset(f'{dir_MMLE_forced}MMLE_ENSforced_ARC_trend_1950-2013_sliding.nc').trend
MMLE_forced_NAWH_trend = xr.open_dataset(f'{dir_MMLE_forced}MMLE_ENSforced_subpolar_gyre_trend_1950-2013_sliding.nc').trend
MMLE_forced_SEP_trend = xr.open_dataset(f'{dir_MMLE_forced}MMLE_ENSforced_SoutheastPacific_trend_1950-2013_sliding.nc').trend
MMLE_forced_SOP_trend = xr.open_dataset(f'{dir_MMLE_forced}MMLE_ENSforced_SOP_trend_1950-2013_sliding.nc').trend
# %%
...
SMILE_unforced_trend = xr.open_dataset('/work/mh0033/m301036/OBS_LPS_revision/docs/data/FIG5/SMILE_residual_ICV_2013/MMLE_equal_weighted_5th_95th_quantile_1950_2013_sliding.nc')
SMILE_unforced_trend
# %%
arctic_SMILE_unforced_lower = SMILE_unforced_trend["q_low"].sel(region='Arctic')
NAWH_SMILE_unforced_lower = SMILE_unforced_trend["q_low"].sel(region='Subpolar_gyre')
SEP_SMILE_unforced_lower = SMILE_unforced_trend["q_low"].sel(region='SoutheastPacific')
SOP_SMILE_unforced_lower = SMILE_unforced_trend["q_low"].sel(region='SOP_original')
arctic_SMILE_unforced_upper = SMILE_unforced_trend["q_high"].sel(region='Arctic')
NAWH_SMILE_unforced_upper = SMILE_unforced_trend["q_high"].sel(region='Subpolar_gyre')
SEP_SMILE_unforced_upper = SMILE_unforced_trend["q_high"].sel(region='SoutheastPacific')
SOP_SMILE_unforced_upper = SMILE_unforced_trend["q_high"].sel(region='SOP_original')
# %%
import seaborn as sns
...
for i, var in enumerate(vars):
    if var == 'MMLE_forced':
        ...
        sns.lineplot(x=np.arange(1950, 2005), y=da_Arctic[var].values, color="#B11927", linestyle=":", linewidth=5.5, ax=ax1)
        sns.lineplot(x=np.arange(1950, 2005), y=da_NorthAtlantic[var].values, color="#B11927", linestyle=":", linewidth=5.5, ax=ax2)
        sns.lineplot(x=np.arange(1950, 2005), y=da_SoutheastPacific[var].values, color="#B11927", linestyle=":", linewidth=5.5, ax=ax3)
        sns.lineplot(x=np.arange(1950, 2005), y=da_SOP[var].values, color="#B11927", linestyle=":", linewidth=5.5, ax=ax4)
    else:
...
ax1.fill_between(np.arange(1950,2005), arctic_unforced_lower.ICV_trend_lower[:55].values[::-1], arctic_unforced_upper.ICV_trend_upper[:55].values[::-1], color=colors[3])
ax2.fill_between(np.arange(1950,2005), NAWH_unforced_lower.ICV_trend_lower[:55].values[::-1], NAWH_unforced_upper.ICV_trend_upper[:55].values[::-1], color=colors[3])
ax3.fill_between(np.arange(1950,2005), SEP_unforced_lower.ICV_trend_lower[:55].values[::-1], SEP_unforced_upper.ICV_trend_upper[:55].values[::-1], color=colors[3])
ax4.fill_between(np.arange(1950,2005), SOP_unforced_lower.ICV_trend_lower[:55].values[::-1], SOP_unforced_upper.ICV_trend_upper[:55].values[::-1], color=colors[3])

# Add the unforced shading region filled with hatched lines
years = np.arange(1950, 2005)

for ax, low, high in [
    (ax1, arctic_SMILE_unforced_lower.values, arctic_SMILE_unforced_upper.values),
    (ax2, NAWH_SMILE_unforced_lower.values,    NAWH_SMILE_unforced_upper.values),
    (ax3, SEP_SMILE_unforced_lower.values,     SEP_SMILE_unforced_upper.values),
    (ax4, SOP_SMILE_unforced_lower.values,     SOP_SMILE_unforced_upper.values),
]:
...
for ax in [ax1, ax2, ax3, ax4]:
    ...
    ax.tick_params(axis='x', labelsize=26, which='major', length=12, width=2.5, direction='in')
    ax.tick_params(axis='x', which='minor', length=8, width=2.5, direction='in')
    ax.tick_params(axis='y', labelsize=26, which='major', length=12, width=2.5, direction='in')
...
ax3_upper = ax3.twiny()
ax4_upper = ax4.twiny()
start_years = np.array([1954, 1964, 1974, 1984, 1994, 2004])
trend_lengths = 2004 - start_years + 10  # [60, 50, 40, 30, 20, 10]
for ax_upper in [ax1_upper, ax2_upper, ax3_upper, ax4_upper]:
    ...
    ax_upper.set_xticklabels(trend_lengths.astype(str))
...
ax1.text(1943, 1.85, 'A', fontsize=35, ha='center', va='center', fontweight='bold')
ax2.text(1943, 1.58, 'B', fontsize=35, ha='center', va='center', fontweight='bold')
ax3.text(1943, 1.58, 'C', fontsize=35, ha='center', va='center', fontweight='bold')
ax4.text(1943, 0.95, 'D', fontsize=35, ha='center', va='center', fontweight='bold')
...
ax2.add_artist(leg2)
dir_fig_output = "/work/mh0033/m301036/OBS_LPS_revision/docs/Figs/FIGS_trend_partition_end2013/"
os.makedirs(dir_fig_output, exist_ok=True)
plt.savefig(f'{dir_fig_output}FIGURE_S12_1950-2013_Jan23.png', dpi=300, bbox_inches='tight')
plt.savefig(f'{dir_fig_output}FIGURE_S12_1950-2013_Jan23.pdf', dpi=300, bbox_inches='tight')
