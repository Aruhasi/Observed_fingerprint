...
# output directory
FIG_OUT_DIR = "/work/mh0033/m301036/OBS_LPS_revision/docs/Figs/FIG_SI_LEs_emergence/"
...
def load_LE_emergence(case="mean"):
    ...
    base = f"/work/mh0033/m301036/OBS_LPS_revision/docs/data/FIG4/{suffix}/"
    ...
    CESM2_ENS     = xr.open_dataset(base + f"CESM_emergence_timescale_{suffix}.nc")
    ...
    for model_name in list(models.keys()):
        ...
        if "emergence_timescale_mean" in ds.data_vars:
            # already fine
        ...
        if "__xarray_dataarray_variable__" in ds.data_vars:
            ds = ds.rename({"__xarray_dataarray_variable__": "emergence_timescale_mean"})
            print(f"[load_LE_emergence] Renamed '__xarray_dataarray_variable__' -> 'emergence_timescale_mean' in {model_name}")
        else:
            # fall back: if there is exactly one data variable, rename that
            data_vars = list(ds.data_vars)
            if len(data_vars) == 1:
                old_name = data_vars[0]
                ds = ds.rename({old_name: "emergence_timescale_mean"})
                print(f"[load_LE_emergence] Renamed '{old_name}' -> 'emergence_timescale_mean' in {model_name}")
            else:
                raise KeyError(
                    f"In model {model_name}, could not find 'emergence_timescale_mean' "
    ...
    stack = xr.concat(
        [ds["emergence_timescale_mean"] for ds in models.values()],
        dim="model",
    )
    stack = stack.assign_coords(model=list(models.keys()))
    MMEM_field = stack.median("model")   # median across models
    MMEM = MMEM_field.to_dataset(name="emergence_timescale_mean")
...
dir_monot = "/work/mh0033/m301036/OBS_LPS_revision/docs/data/FIG4/monotonicity/"

MPI_ESM_monotonicity   = xr.open_dataset(dir_monot + "MPI_ESM_emergence_monotonicity.nc")
MIROC6_monotonicity    = xr.open_dataset(dir_monot + "MIROC6_emergence_monotonicity.nc")
ACCESS_monotonicity    = xr.open_dataset(dir_monot + "ACCESS_emergence_monotonicity.nc")
EC_Earth3_monotonicity = xr.open_dataset(dir_monot + "EC_Earth3_emergence_monotonicity.nc")
IPSL_CM6A_monotonicity = xr.open_dataset(dir_monot + "IPSL_CM6A_emergence_monotonicity.nc")
CESM2_monotonicity     = xr.open_dataset(dir_monot + "CESM_emergence_monotonicity.nc")
CanESM5_monotonicity   = xr.open_dataset(dir_monot + "CanESM5_emergence_monotonicity.nc")
...
ax_mmem.text(0.5, 1.0, "MMLE", transform=ax_mmem.transAxes,
             fontsize=26, ha="center", va="bottom")
ax_mmem.text(-0.05, 1.05, "A", transform=ax_mmem.transAxes,
             fontsize=26, fontweight="bold", ha="right", va="bottom")

im = MMEM["emergence_timescale_mean"].plot(
    ax=ax_mmem,
...
for idx, (ax, title, data, label) in enumerate(zip(model_axes, model_titles, model_data, subplot_labels)):
    ...
    data["emergence_timescale_mean"].plot(
        ax=ax,
        transform=ccrs.PlateCarree(),
        cmap=custom_cmap,
        levels=levels,
...
def draw_boxes(ax, text_size):
    ...
    ax.text(sep_lon_mid, sep_lat_mid, "SEP", color="black", fontsize=text_size,
            transform=ccrs.PlateCarree(), ha="center", va="center")
    # SOP box
    ax.plot([230%360, 280%360, 280%360, 230%360, 230%360], 
            [-40, -40, -70, -70, -40], color='gray', linewidth=3.0, transform=ccrs.PlateCarree())
...
cbar.set_label("Emergence timescale with S/N > 2 (year)", fontsize=24, loc="center")
...
tag = f"{scen}_{case}"
for ext in ("png", "pdf"):
    fname = f"FIGURE_S11_emergence_timescale_LEs_{tag}.{ext}"
