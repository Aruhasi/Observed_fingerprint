...
in_file = f"{DIR_IN}/SAT_anomaly_partition_wrt_{model}_CMIP6_SMBB_ENS_GSAT_1850-2022.nc"
...
# Drop non-numeric/object coords that can break NetCDF encoding (e.g., 'realization')
for coord_to_drop in ("realization",):
    if coord_to_drop in tas_run.coords:
        tas_run = tas_run.drop_vars(coord_to_drop)
...
trend_all, pval_all = xr.apply_ufunc(
    ...
    vectorize=True,
    dask="parallelized",
    dask_gufunc_kwargs={"allow_rechunk": True, "output_sizes": {"period": len(period_starts)}},
...
# Ensure coords are basic dtypes for safe NetCDF encoding
for coord_name in list(ds_out.coords):
    if ds_out[coord_name].dtype == object:
        ds_out[coord_name] = ds_out[coord_name].astype(str)
