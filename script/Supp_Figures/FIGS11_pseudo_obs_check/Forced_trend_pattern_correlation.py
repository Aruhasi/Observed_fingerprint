...
# %%
dir_input = '/work/mh0033/m301036/OBS_LPS_revision/docs/data/FIG3/MPI_ESM/SMILE_forced/'
run_indir = "/work/mh0033/m301036/OBS_LPS_revision/docs/data/Revision_check/pseudo_obs_check/trend/forced/"
model_names = ["MIROC6", "ACCESS", "EC_Earth", "IPSL_CM6A", "CanESM5", "CESM2","MMEM"]

# === Load ensemble mean trend ===
ensemble_ds = xr.open_dataset(os.path.join(dir_input, 'MPI_ESM_ENSmean_forced_MK_trend_1950-2022_sliding.nc'))
# %%
ensemble_ds = ensemble_ds.sel(period='1993-2022')
# %%
ensemble_trend = ensemble_ds['trend']  # shape: (lat, lon)
# %%
...
for model in model_names:
    file_path = os.path.join(run_indir, f"MPI_ESM_tas_forced_trend_30yr_wrt_{model}_1993_2022.nc")
    ds_model = xr.open_dataset(file_path)  # shape: (run, lat, lon)

    correlations = []
    for run in ds_model.run.values:
        run_data = ds_model.sel(run=run)['30yr_forced'] 
...
for model, corrs in correlation_results.items():
    file_out = os.path.join(run_indir, f"{model}_correlation_with_ensemble.txt")
