...
def compute_trend_for_segment_length(data, seg_len, trend_function):
    """
    Compute MK trend for one segment length without materializing all segments.
    """
...
def compute_trend(data, segment_lengths, trend_function):
    ...
    # pad to common segment length
    padded_ds = xr.Dataset()
    for name, da in ICV_segments_ds.data_vars.items():
        num_seg = da.sizes["segment"]
        if num_seg < max_segments:
            padding = xr.DataArray(
                np.full((max_segments - num_seg, *da.shape[1:]), np.nan, dtype=da.dtype),
                dims=["segment", *da.dims[1:]],
                coords={**da.coords, "segment": np.arange(num_seg, max_segments)},
            )
            padded_ds[name] = xr.concat([da, padding], dim="segment")
        else:
            padded_ds[name] = da
    return padded_ds
# %%
def process_realization(run):
    ...
    run_file = f"{dir_out}/ICV_{model}_MK_trend_{start_year}-{end_year}_segments_run{run}.nc"
    combined_results.to_netcdf(run_file)
    print(f"[rank {rank}] Saved run {run} to {run_file}", flush=True)
...
for run in run_single:
    ...
    run_file = process_realization(run)
    saved_files.append(run_file)

if saved_files:
    print(f"[rank {rank}] Saved {len(saved_files)} run files", flush=True)
else:
...
# ----------------- merge on rank 0 -----------------
if rank == 0:
    print(f"All ranks finished. Run files saved to {dir_out}/ICV_{model}_MK_trend_*.nc", flush=True)
# if rank == 0:
