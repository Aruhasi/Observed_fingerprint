...
# -*- coding: utf-8 -*-
"""
Simple (unweighted) Pearson pattern RMSE between SMILE realizations /
observations and SMILE ensemble-mean internal trend patterns for sliding
windows 1950–2022...2013–2022 (10–73 years).

For a given model:

1) For each run and each sliding period:
     - Compute Pearson pattern correlation with the SMILE ensemble-mean internal
         trend pattern for that period.

2) For each period:
     - Compute Pearson pattern correlation between observed internal trend
         pattern and the SMILE ensemble-mean internal trend pattern.

Outputs (one NetCDF per model):

- pattern_rms_run_vs_mmem(run, period)
- pattern_rms_obs_vs_mmem(period)
...
import src.Statistic_cal as stats_calc
# %%
# ---------------------------------------------------------------------
# MPI setup
# ---------------------------------------------------------------------
...
if len(sys.argv) < 3:
    if rank == 0:
        print(
            "Usage: python Setp4_pattern_RMSE_MPI.py <MODEL_NAME> <OBS_internal_FILE>",
            flush=True,
        )
    sys.exit(1)
...
# ---------------------------------------------------------------------
def load_MMLE_internal(model_name: str):
...
# ---------------------------------------------------------------------
def compute_run_vs_mmem_RMSE(tas_mem: xr.DataArray,
                                     internal_trend: xr.DataArray) -> np.ndarray:
    """
    Compute Pearson pattern correlation between each run's MK trend map and the
    SMILE ensemble-mean internal trend map for each sliding period.

    tas_mem: (run, year, lat, lon) annual anomalies
    internal_trend: (period, lat, lon) MK trend (K/decade)

    Returns (on rank 0): rms_global[run, period]
    """
    ...
    rms_local = np.full((n_runs_total, n_period), np.nan, dtype=np.float64)
    ...
    for ir in local_indices:
        ...
        for ip in range(n_period):
            ...
            # Pattern correlation
            rms_val = stats_calc.pattern_rmse(
                LE_trend_map, internal_map, 
                lat=LE_trend_map["lat"],
                dims=("lat", "lon"),
                use_weights=False,
                centered=False
                )
            rms_local[ir, ip] = rms_val

    # Gather all local pieces to rank 0
    rms_list = comm.gather(rms_local, root=0)

    if rank == 0:
        # Combine by taking nanmax across ranks (only one non-NaN per entry)
        rms_stack = np.stack(rms_list, axis=0)  # (npro, n_runs, n_period)
        rms_global = np.nanmax(rms_stack, axis=0)  # (n_runs, n_period)
        return rms_global
    else:
        return None
# %%
...
# ---------------------------------------------------------------------
def compute_obs_vs_mmem_RMSE(obs_trend: xr.DataArray,
                                     internal_trend: xr.DataArray) -> np.ndarray:
    """
    Compute Pearson pattern correlation between observed internal trend and
    SMILE ensemble-mean internal trend, for each period.

    obs_trend: (period, lat, lon)
    internal_trend: (period, lat, lon)

    Returns: rms_obs[period]
    """
    rms_obs = np.full((n_period,), np.nan, dtype=np.float64)
    ...
    for ip, period_label in enumerate(period_labels):
        ...
        rms_obs[ip] = stats_calc.pattern_rmse(
                obs_map, internal_map, 
                lat=obs_map["lat"],
                dims=("lat", "lon"),
                use_weights=False,
                centered=False
...
tas_mem, internal_trend = load_members_and_internal(model)
...
rms_global = compute_run_vs_mmem_RMSE(tas_mem, internal_trend)
...
if rank == 0:
    ...
    # Compute obs vs MMEM RMSE
    rms_obs = compute_obs_vs_mmem_RMSE(obs_trend, internal_trend)

    # Build output dataset
    runs = tas_mem["run"].values
    rms_run_da = xr.DataArray(
        rms_global,
        dims=("run", "period"),
        coords={"run": runs, "period": period_labels},
        name="pattern_rms_run_vs_mmem",
    )
    rms_run_da.attrs["description"] = (
        "Pearson pattern correlation between each run's MK trend and the SMILE "
        "ensemble-mean internal MK trend, for sliding windows "
        "1950–2022...2013–2022 (10–73 years)."
    )

    rms_obs_da = xr.DataArray(
        rms_obs,
        dims=("period",),
        coords={"period": period_labels},
        name="pattern_rms_obs_vs_mmem",
    )
    rms_obs_da.attrs["description"] = (
        "Pearson pattern correlation between observed internal MK trend and the "
        "SMILE ensemble-mean internal MK trend, for sliding windows "
        "1950–2022...2013–2022 (10–73 years)."
    )

    ds_out = xr.Dataset(
        {
            "pattern_rmse_run_vs_mmem": rms_run_da,
            "pattern_rmse_obs_vs_mmem": rms_obs_da,
        }
    )

    out_file = os.path.join(
        dir_out, f"{model}_ICVstd_pattern_rmse_run_and_obs_vs_MMEM_1950-2022_sliding.nc"
    )
