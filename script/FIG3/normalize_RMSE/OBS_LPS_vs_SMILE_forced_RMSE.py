...
# %%
# ---------------------------------------------------------------------
# 1) Pattern RMSE: each run vs SMILE MMEM forced trend
# ---------------------------------------------------------------------
...
# %%
# ---------------------------------------------------------------------
# 2) Pattern rmselation: observed forced vs SMILE MMEM forced
# ---------------------------------------------------------------------
...
# Compute pattern RMSE: run vs MMEM
rms_global = compute_run_vs_mmem_RMSE(tas_mem, forced_trend)
...
if rank == 0:
    ...
    rms_obs = compute_obs_vs_mmem_RMSE(obs_trend, forced_trend)
    ...
    rms_run_da = xr.DataArray(
        ...
        name="pattern_rmse_run_vs_mmem",
    )
    rms_run_da.attrs["description"] = (
        "Root mean square error between each run's MK trend and the SMILE "
    ...
    rms_obs_da = xr.DataArray(
        ...
        name="pattern_rmse_obs_vs_mmem",
    )
    rms_obs_da.attrs["description"] = (
        "Root mean square error between observed forced MK trend and the "
    ...
    out_file = os.path.join(
        dir_out, f"{model}_forced_pattern_rmse_run_and_obs_vs_MMEM_1950-2022_sliding.nc"
    )
